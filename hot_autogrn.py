...
def get_excel_files_with_grn(drive_service, folder_id, page_size=1000):
    """Get Excel files with 'GRN' in the name, with pagination support"""
    files = []
    next_page_token = None
    
    while True:
        # Query for Excel files containing 'GRN' in the name
        query = f"'{folder_id}' in parents and (mimeType='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' or mimeType='application/vnd.ms-excel') and name contains 'GRN'"
        ...
        print(f"Found {len(batch_files)} GRN files in this batch (Total so far: {len(files)})")
...
def try_raw_xml_extraction(file_stream, header_row):
    ...
    try:
        ...
        with zipfile.ZipFile(file_stream, 'r') as zip_ref:
            ...
            # Look for shared strings (where text is stored)
            shared_strings = {}
            ...
            with zip_ref.open(worksheet_files[0]) as xml_file:
                ...
                # Use regex to extract cell references, types, and values
                import re
                
                # Find all cell references with their types and values
                # Look for cells with type="s" (shared string), type="str" (inline string), or no type (number)
...
def read_excel_file(drive_service, file_id, filename, header_row):
    ...
    # Strategy 0: Try openpyxl engine first (best for .xlsx files with text)
    try:
        file_stream.seek(0)
        if header_row == -1:
    ...
    # Strategy 1: Try xlrd for .xls files
    ...
    # Strategy 2: Try calamine engine (if available)
    try:
        file_stream.seek(0)
        if header_row == -1:
    ...
    # Strategy 3: Try alternative engines
    engines_to_try = []
    ...
    # Strategy 4: Try xlwings (if on Windows/Mac with Excel installed)
    ...
    # Strategy 5: Try raw XML extraction (most aggressive) - now with proper text handling
    ...
    # Strategy 6: Try external conversion tools
    ...
    # Strategy 7: Last resort - try to extract any readable text
    try:
        file_stream.seek(0)
        with zipfile.ZipFile(file_stream, 'r') as zip_ref:
...
def append_to_sheet(sheets_service, spreadsheet_id, sheet_name, data, append_headers):
    try:
...
def main():
    # Configuration
    FOLDER_ID = '1-Gyaxv4WYjmOI4v2BBx9hDT37kcLweBY'
    SPREADSHEET_ID = '10wyfALowemBcEFiZP9Tyy08npl_44FpHonO3rKARmRY'
    SHEET_NAME = 'hotgrn'
    BATCH_SIZE = 50  # Process 50 files at once

    print("Enhanced Excel Reader v3.0 - GRN File Filter & Batch Processing")
    ...
    # Get list of Excel files with 'GRN' in the name
    print(f"\nSearching for Excel files containing 'GRN' in the name...")
    excel_files = get_excel_files_with_grn(drive_service, FOLDER_ID, page_size=BATCH_SIZE)
    
    if not excel_files:
        print("No Excel files with 'GRN' in the name found in the specified folder.")
        return

    print(f"\nFound {len(excel_files)} Excel files containing 'GRN':")
    for i, file in enumerate(excel_files[:10]):  # Show first 10 files
        print(f"  - {file['name']}")
    ...
    for batch, batch_num, total_batches in process_files_in_batches(excel_files, BATCH_SIZE):
        ...
        # Process each Excel file in the current batch
        for i, file in enumerate(batch, 1):
            file_index = ((batch_num - 1) * BATCH_SIZE) + i
            print(f"\n[Batch {batch_num}/{total_batches}] [{i}/{len(batch)}] [Overall: {file_index}/{len(excel_files)}] Processing: {file['name']}")
            ...
            try:
                ...
                append_to_sheet(sheets_service, SPREADSHEET_ID, SHEET_NAME, df, is_first_file)
            ...
            def remove_duplicates_from_sheet(sheets_service, spreadsheet_id, sheet_name):
                ...
                try:
                    ...
                    # Deduplicate only on Item Code + po_number
                    if "Item Code" in df.columns and "po_number" in df.columns:
                        df = df.drop_duplicates(subset=["Item Code", "po_number"], keep="first")
                        after = len(df)
                        removed = before - after
                    else:
                        print("⚠️ Warning: 'Item Code' or 'po_number' column not found, skipping duplicate removal.")
                        removed = 0
                        after = before
    ...
    print(f"{'='*60}")
    print(f"Total files found with 'GRN': {len(excel_files)}")
    ...
    print(f"Files processed in {((len(excel_files) + BATCH_SIZE - 1) // BATCH_SIZE)} batches of {BATCH_SIZE}")
        # 🔥 Final cleanup: remove duplicates from the sheet
